...
(() => {
  ...
  LineLib.reg_property = function(solid, metaName, propName = null, constructor = String, a_default =
  null) {
    ...
    try {
      ...
      var factory_is_type = typeof(constructor) != "undefined" && typeof(constructor) != "function" &&
        constructor.prototype && constructor.prototype instanceof Object;
      ...
      var setit = function(prop) {
        ...
        var setnew = function(newobj) {
          console.log(
            `${propName} -> ${typeof(newobj)} ${newobj == null ? "!" : (typeof(newobj) == typeof([]) ? mydump(newobj) : newobj)}`
            );
          if (typeof(newobj) != 'undefined') {
            if (!solidsProps.has(propName)) {
              ...
              console.log(
                `Wont set prop ${propName} -> ${newobj == null ? "!" : (typeof(newobj) == typeof([]) ? mydump(newobj) : newobj)}`
                );
            }
          } else {
            console.warn(
              `Setting ${propName} ${mydump(prop)}-> undefined ${newobj} from constructor ${constructor})`
              );
        ...
        if (solidsProps.has(propName) && (typeof(prop) == 'undefined' || prop == null)) {
          ...
          if (factory_is_type) {
            try {
              console.log(
                `Calling constructor as Type "${constructor}" "${constructor.prototype}" for prop ${mydump(prop)}`
                );
            ...
            try {
              ...
              console.warn(
                `Failed setting prop ${propName} ${mydump(prop)} w/function ${constructor} registration ${e.message}`,
                e);
            }
          } else {
      ...
      if (typeof(solid_prop) != "undefined") {
        try {
          console.log(
            `Register property ${metaName} as ${propName} from user value: ${mydump(solid_prop)}`);
          setit(solid_prop);
          return (solidsProps);
        } catch (e) {
          /* still try to supply the default if failed to use the provied property from the solid */
          console.warn(
            `Failed Attempt to use default_value from supplied ${a_default != null && a_default.prototype && a_default.prototype instanceof Object ? "type: " + typeof(a_default) : (typeof(a_default) == "function" ? a_default : mydump(a_default))}`
            );
        }
      }
      ...
      try {
        ...
        console.log(
          `Register property ${metaName} as ${propName} from default object Value : ${typeof(default_value)} ${mydump(default_value)}`
          );
        setit(default_value);
        return solidsProps;
      } catch (e) {
        console.warn(
          `Failed Attempt to use default_value from supplied ${a_default && a_default.prototype && a_default.prototype instanceof Object ? "type: " + typeof(a_default) : (typeof(a_default) == "function" ? a_default : mydump(a_default))}`
          );
      }
    } catch (e) {
  ...
  LineLib.lineProps = function(solid) {
        ...
        try {
          if (typeof(solid_prop) != "undefined" && solid_prop) {
            ...
            if (ts && func && func.prototype && func.prototype instanceof Object) {
              ...
              return [].concat(ts.map((l) => {
                return func(l, solidsProps)
              }));
            } else if (typeof(ts) == typeof([])) {
              return [].concat(ts)
            }
          } else {
            console.error("solid_prop undefined")
          }
        } catch (e) {
    ...
    function parseObject(obj, baseClassName, nullvalue, defValue) {
      ...
      if ((typeof(nullvalue) != "undefined") && (
          (typeof(nullvalue) == typeof([]) && nullvalue.some((n) => {
            return (n == baseClassName);
          })) ||
          (typeof(nullvalue) == typeof(obj) && obj == nullvalue)
        )) {
        console.log(`nullable found "${obj}" is in ${mydump(nullvalue)}`);
        return (null);
      }
      ...
      return (Function(
        `"use strict"; return({newobj: ${baseClassName}.${typeof(obj) != "undefined" ? obj : defValue}});`
        )().newobj);
    }

    function parseColor(obj) {
      // Evals a Color as text, if the input is "", short circuit
    ...
    var solidsProps = this.reg_property(solid, "line_height_offsets", "height_offsets", an_array(
      parseDecimal));
    /* a majorSpaing  */
    ...
    /* load in spacing settins manually.
 ...
 linewidth = spacing / pitch
    */
    ...
    this.reg_property(solid, "spacing", "spacing", (p, solidsProps) => {
      if (!solidsProps.has("pitch")) {
        solidsProps.set("pitch", 1)
      }
  ...
  LineLib.fillRect = function(canvas, solid) {
    ...
    for (let [key, value] of props.entries()) {
      console.log(`${key} ${typeof(value) != 'undefined' ? mydump(value) : ""}`)
    }
    /* Locks the original object if poperty "lock" will be set to the evalulation
     * of Boolean(x) where x is the solid metadata for "lock". See function
     * LineLib.lineProps() for more detaionls
     */
    var lock = props.get("lock") || false;
    /* Calculate spacing for lines using

       "spacing" "majorSpacing" "steps" "pitch" of a solid
       ...
       MajorSpacing: can be overriden with the property "majorSpacing" but this
       is truly only supported so the created solids could recreate themselves
       independent of current canvas setting

       Steps: number of steps to fit in the same height as the MajorSpacing or
       canvase's majorSpacing

       Spacing: the space between lines which should be = canvas.grid.spacing
       unless you changed any of the above

       line_spacing: the final calculated value for all this. It will be set to
       "+line_Spacing" in the placeholder sold it creates. NOTE: it's set with a
       proceeding + because this will not be used to create solids, just for
       scaling margins

       x_end, y_end: are the sentry for the line creation
    ...
    var hColor = props.has("line_color") ? (props.get("line_color") == null ? (solid.strokeColor != Color
      .clear ? solid.strokeColor : Color.black) : props.get("line_color")) : Color.black;
    var vColor = props.has("vertical_color") ? (props.get("vertical_color") == null ? (solid
      .strokeColor != Color.clear ? solid.strokeColor : Color.Black) : hColor) : Color.black;
    ...
    if (props.has("vertical_offsets") || props.has("vertical_margin") || props.has('vertical_sizes')) {
      ...
      var vOnce = !["vertical_offsets", "vertical_types", "vertical_sizes"].some((x) => {
        return (typeof(props.get(x)) == typeof([]))
      });
      var vcumulative_offset = 0;
      var voffset = vMargin;
      var line_x = startx + voffset;
      console.log("Starting vertical lines: line_x(" + line_x.toFixed(3) + ") = startx(" + startx +
        ") < y_end(" + y_end + ") once:" + vOnce);
      for (var line_num = 0; startx + vcumulative_offset + (line_num * line_spacing) + voffset <=
        y_end; line_num++) {
        ...
        if (vlineTypes.length > 0) {
          line.name +=
            `type(${l},${vlineTypes.length >1 && vlineTypes[1] == '...' ? "!" : ""}${Math.floor(l % vlineTypes.length)}/${vlineTypes.length}):${vlineType}`
        }
        if (vSizes.length > 0) {
          line.name +=
            `size:(${p},${vSizes.length >1 && vSizes[1] == '...' ? "!" : ""}${Math.floor(p % vSizes.length)}/${vSizes.length}):${vSizes.length ? vSizes[0] : 1.0}, `
        }
        if (vdistances.length > 0) {
          line.name +=
            `offest(${t},${vdistances.length >1 && vdistances[1] == '...' ? "!" : ""}${Math.floor(t % vdistances.length)}/${vdistances.length}):${voffset},`
        }
        line.name +=
          `color:${(!vSizes.length || ![0.0,0,null].some((x) => vSizes[0]==x))}}, startx(${startx})+⨊offset(${vcumulative_offset-voffset}) + spacing(${(line_num * line_spacing) + voffset }) = ${line_x} < y_end(${y_end})`;

        line.strokeColor = (!vSizes.length || ![0.0, 0, null].some((x) => vSizes[0] == x)) ? vColor :
          Color.clear;
        ...
        if (vSizes.length > 1 && vSizes[1] != '...') {
          vSizes.rotate();
          p += 1
        }
        if (vlineTypes.length > 1 && vlineTypes[1] != '...') {
          vlineTypes.rotate();
          l += 1
        }
        if (vdistances.length > 1 && vdistances[1] != '...' && line_num > 0) {
          vdistances.rotate();
          t += 1
        } else if (vdistances[0] == '...') {
          vdistances.rotate(true);
          t -= 1;
        }
        // set after we rotate
        voffset = vdistances.length && vdistances[0] != '...' ? vdistances[0] : 0.0;
        console.log("line_x(" + line_x.toFixed(3) + ") = startx(" + startx + ") + offset(" +
          vcumulative_offset + ") + " + (line_num * line_spacing) + " + voffset(" + voffset +
          ") < x_end(" + x_end + ")");
      }
    }
    if (!skip_horizontal) {
      ...
      p = 0;
      l = 0;
      console.log("starting line_y(" + line_y.toFixed(3) + ") = starty(" + starty + ") + offset(" +
        cumulative_offset + ") +  offset(" + offset + ") < x_end" + x_end + ")");
      for (line_num = 0; starty + cumulative_offset + (line_num * line_spacing) + offset <=
        x_end; line_num++) {
        /* run away? */
        if (line_num >= 4096) {
          break;
        ...
        if (lineTypes.length > 0) {
          line.name +=
            `type(${l}${lineTypes.length >1 && lineTypes[1] == '...' ? "!" : ""},${Math.floor(l % lineTypes.length)}/${lineTypes.length}):${lineType}`
        }
        if (sizes.length > 0) {
          line.name +=
            `size:(${p}${sizes.length >1 && sizes[1] == '...' ? "!" : ""},${Math.floor(p % sizes.length)}/${sizes.length}):${size}, `
        }
        if (distances.length > 0) {
          line.name +=
            `offest(${t}${distances.length >1 && distances[1] == '...' ? "!" : ""},${Math.floor(t % distances.length)}/${distances.length}):${offset},`
        }
        line.name +=
          `color:${(!sizes.length || ![0.0,0,null].some((x) => sizes[0]==x))}} starty(${starty})+⨊offset(${cumulative_offset-offset}) + spacing(${(line_num * line_spacing) + offset }) = ${line_y} < x_end(${x_end})`;

        line.strokeColor = (!sizes.length || ![0.0, 0, null].some((x) => sizes[0] == x)) ? hColor : Color
          .clear;
        ...
        if (lineTypes.length > 1 && lineTypes[1] != '...') {
          lineTypes.rotate();
          l += 1
        }
        if (distances.length > 1 && distances[1] != '...' && line_num > 0) {
          distances.rotate();
          t += 1
        } else if (distances[0] == '...') {
          vdistances.rotate(true);
        ...
        console.log("line_y(" + line_y.toFixed(3) + ") = starty(" + starty + ") + offset(" +
          cumulative_offset + ") + " + (line_num * line_spacing) + " + offset(" + voffset +
          ") < x_end(" + x_end + ")");
      }
